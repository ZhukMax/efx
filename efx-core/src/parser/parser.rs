...
use crate::parser::nodes::{Element, Interpolation, Node, Text, Attr};
        ...
        match self.cur().cloned() {
            Some((Tok::Text(s), sp)) => {
                ...
                if !remainder.is_empty() {
                    ...
                    self.toks.insert(self.i, (Tok::Text(remainder.to_string()), SpanRange::new(rem_start, sp.end.0)));
                }
            ...
            match self.cur() {
                Some((Tok::Slash, _)) => {
                    return if let Some((Tok::RAngle, sp_gt)) = self.toks.get(self.i + 1).cloned() {
                        ...
                        let end = sp_gt.end;
                        let attrs = parse_attrs_from_buffer(&attrs_src, SpanRange {
                        ...
                        Err(ParseError { msg: "unexpected '/' in tag head".into(), span: sp })
                    }
                }
                        ...
                        match self.cur() {
                            Some((Tok::LAngle, _)) => {
                                if let Some((Tok::Slash, _)) = self.toks.get(self.i + 1) {
                                    ...
                                    return Ok(Node::Element(Element { name, attrs, children, span: SpanRange { start, end: end_angle.end } }));
                                } else {
                ...
                Some((Tok::Text(s), sp)) => {
                    ...
                    if attrs_span_start.is_none() { attrs_span_start = Some(sp.start); }
                    attrs_span_end = Some(sp.end);
                    attrs_src.push_str(s);
                    self.bump();
                }
                Some((tok, sp)) => {
                    return Err(ParseError { msg: format!("unexpected token in tag head: {:?}", tok), span: *sp });
                }
                None => {
                    return Err(ParseError { msg: "unexpected EOF in tag head".into(), span: SpanRange::new(self.byte_end(), self.byte_end()) });
                }
            }
        }
    }
...
fn parse_attrs_from_buffer(buf: &str, span: SpanRange) -> PResult<Vec<Attr>> {
    ...
    let skip_ws = |b: &[u8], mut j: usize| { while j < b.len() && b[j].is_ascii_whitespace() { j += 1; } j };
    let is_name_char = |c: u8| -> bool { c.is_ascii_alphanumeric() || c == b'_' || c == b'-' || c == b':' };
    ...
    while i < bytes.len() {
        ...
        if i == name_start {
            ...
            return Err(ParseError { msg: "invalid attribute name".into(), span });
        }

        let name = &buf[name_start..i];
        i = skip_ws(bytes, i);
        if i >= bytes.len() || bytes[i] != b'=' {
            return Err(ParseError { msg: format!("expected '=' after attribute '{}'", name), span });
        }
        i += 1;
        i = skip_ws(bytes, i);

        if i >= bytes.len() || (bytes[i] != b'"' && bytes[i] != b'\'') {
            return Err(ParseError { msg: format!("expected quote after '=' in attribute '{}'", name), span });
        }
        let quote = bytes[i];
        i += 1;
        let val_start = i;
        ...
        if i >= bytes.len() {
            return Err(ParseError { msg: format!("unterminated quoted value for attribute '{}'", name), span });
        }
